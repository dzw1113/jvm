...
public class CFrame_Jackie extends javax.swing.JFrame {
	...
	public CFrame_Jackie(C_Jackie client) {
		this.client = client;
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 1) {
					if (!Morechooes) { // 单发状态
						...
						Jackie_ReadClient.isNewMessage = false;//解除托盘闪动状态//解除托盘闪动状态
		...
		if (!client.isInterruptWithServer()) { // 先判断是否服务器先关闭
			if (client.getSocket() == null) {
				...
				try {
					...
					Jackie_ReadClient.socket.close();
					isOnline = false;
			...
			Jackie_ReadClient.socket.close(); // 释放UDP的端口
	...
	public static void main(String args[]) throws InterruptedException, AWTException {
		...
		C_Jackie client = new C_Jackie();
		CFrame_Jackie clientFrame = new CFrame_Jackie(client);
		clientFrame.setTitle("Client_Jackie");
		clientFrame.setResizable(false);
		clientFrame.setLocation(350, 200);
		clientFrame.setVisible(true);
		image = clientFrame.getToolkit().getImage(clientFrame.getClass().getClassLoader().getResource("net/img/Jackie.gif"));
	...
	public void createTrayIcon(final CFrame_Jackie clientFrame) {
		...
		mi.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Jackie_ReadClient.isNewMessage = false;//解除托盘闪动状态
...
class SendFile implements Runnable {
	...
	public SendFile(C_Jackie client, javax.swing.JFileChooser jFileChooser) {
		this.client = client;
	...
	public void run() {
		//传文件的步骤
		int count = CFrame_Jackie.file_port_count;
		User2 c = CFrame_Jackie.vt.get(CFrame_Jackie.index);
		...
		if (info.contains("sureRec"))// 开始发送
		{
			...
			send.start();
			CFrame_Jackie.jTextArea2.append("您向用户 " + c.toString() +"发送文件：" + jFileChooser.getSelectedFile().getName()+"\r\n");
		}
		socket2.close();
	}
...
class SendPoint extends Thread { // 异步_登录显示点
	public void run() {
		while (CFrame_Jackie.sendPoint)// 当操作完成就停止这个
		{
			try {
				Thread.sleep(100);
			} catch (InterruptedException e) {
			...
			CFrame_Jackie.jTextArea2.append("…");
...
class notify2 implements Runnable {  //托盘闪动
	...
	public void run() {
		...
		while (true) {
			if (Jackie_ReadClient.isNewMessage) {
				try {
					CFrame_Jackie.trayIcon.setImage(new ImageIcon("").getImage());
					Thread.sleep(400);
					CFrame_Jackie.trayIcon.setImage(CFrame_Jackie.image);
					Thread.sleep(400);
				} catch (Exception e) {
				}
			} else
