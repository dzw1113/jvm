...
public class C_Jackie implements Runnable {
	...
	public void run() { // 手动登录，后面有一个重新连接（类似）
		try {
			...
			CFrame_Jackie.sendPoint = false; // 不发点了
			...
			if (socket.isConnected() && !socket.isClosed()) {
				Thread.sleep(1000);
				flagString = "******************成功连接服务器******************";
				CFrame_Jackie.jTextArea2.append(flagString + "\r\n");
				CFrame_Jackie.jTextArea2.selectAll();
				CFrame_Jackie.isOnline = true; // 判断是否在线
				CFrame_Jackie.UDPExit = false; // 判断UDP是否监听
				...
				// 接收服务器信息的线程
				Thread thread = new Thread(new Jackie_Read(this));
				thread.start();
				//接收客户端发送信息的线程
				Thread thread2 = new Thread(new Jackie_ReadClient());
				thread2.start();
			...
			CFrame_Jackie.jTextArea2.append("Error：连接超时......\r\n");
			CFrame_Jackie.jTextArea2.selectAll();
			System.out.println("连接超时");
			CFrame_Jackie.flag = true; // flag用来控制能否点登录按钮
	...
	public void write(String send) {
		try {
			...
			CFrame_Jackie.jTextArea2.append("已经与客户端断开连接，发送失败...\r\n");
			CFrame_Jackie.jTextArea2.selectAll();
		}
	}
			...
			for (String string = in.readLine(); string != null; string = in.readLine()) {
				if (string.equals("qwertyuiop[]")) {
					...
					if (!ClientUsername.equals(username)) {
						CFrame_Jackie.jTextArea2.append("您的好友 " + ClientUsername + " 已上线！\r\n");
					}
					...
					if (!ClientUsername.equals(username)) {
						CFrame_Jackie.jTextArea2.append("您的好友 " + ClientUsername + " 已下线！\r\n");
					}
					...
					CFrame_Jackie.jTextArea2.append(string + "\r\n"); // 收到信息
					CFrame_Jackie.jTextArea2.selectAll();// 注意！
				}
			}
		} catch (IOException e) {
			if (!CFrame_Jackie.ClientExit) {
				CFrame_Jackie.jTextArea2.append("提示（1）：服务器已中断……不影响与好友通信\r\n");
				CFrame_Jackie.jTextArea2.append("提示（2）：与服务器失去连接，失去好友下线提示功能\r\n");
				CFrame_Jackie.jTextArea2.append("提示（3）：重新登录并且连上服务器才能刷新好友列表\r\n");
				CFrame_Jackie.jTextArea2.selectAll();
...
class Jackie_Read implements Runnable {
	private C_Jackie client;

	public Jackie_Read(C_Jackie client) {
		this.client = client;
	}
...
class Jackie_ReadClient implements Runnable { // UDP通信
	...
	public void run() {
		...
		try {
			socket = new DatagramSocket(5553); // 监听这个端口
		...
		while (true) {
			if (!socket.isClosed()) {
				...
				try {
					...
					int temp3 = CFrame_Jackie.vt1.indexOf(temp2);

					Thread notifyThread = new Thread(new notify(temp3));
					notifyThread.start();
					CFrame_Jackie.jTextArea2.append("提示： " + temp2 + "向您发了一条消息……\r\n");
					CFrame_Jackie.vt.get(temp3).setstring(info + "\r\n");
					CFrame_Jackie.jTextArea2.selectAll();
				} catch (IOException e) {
...
class recFile implements Runnable {
	...
	public void run() {
		...
		if (result == 0) {// 确认接收文件
			int count = ++Jackie_ReadClient.recFile_port_count;
			byte[] bufsend = ("sureRec" + (5553 + count)).getBytes();
			...
			FileRec server = new FileRec("localhost", 5553 + count, remotePort, dirname + "\\" + filename);
			server.start();
			CFrame_Jackie.jTextArea2.append("文件 \"" + filename + "\" 接收成功！提示：存储在D盘\r\n");
		} else {
...
class notify implements Runnable {
	...
	public void run() {
		for(int i = 0;i<5;i++){  //闪动五次
			CFrame_Jackie.jList1.setSelectedIndex(index);
			try {
				Thread.sleep(90);
			} catch (InterruptedException e) {
			}
			CFrame_Jackie.jList1.clearSelection();
			try {
				Thread.sleep(90);
			} catch (InterruptedException e) {
			}
		...
		if (CFrame_Jackie.index == 0) {
			CFrame_Jackie.jList1.clearSelection();
		} else {
			CFrame_Jackie.jList1.setSelectedIndex(CFrame_Jackie.index);
		}
	}
}