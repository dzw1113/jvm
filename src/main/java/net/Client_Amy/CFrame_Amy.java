...
public class CFrame_Amy extends javax.swing.JFrame {
	...
	public CFrame_Amy(C_Amy client) {
		this.client = client;
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 1) {
					if (!Morechooes) { // 单发状态
						...
						Amy_ReadClient.isNewMessage = false;//解除托盘闪动状态//解除托盘闪动状态
	...
	private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) throws IOException, InterruptedException {
		...
		if (!client.isInterruptWithServer()) { // 先判断是否服务器先关闭
			...
			Amy_ReadClient.socket.close(); // 释放UDP的端口
	...
	public static void main(String args[]) throws InterruptedException, AWTException {
		...
		C_Amy client = new C_Amy();
		CFrame_Amy clientFrame = new CFrame_Amy(client);
		clientFrame.setTitle("Client_Amy");
		clientFrame.setResizable(false);
		clientFrame.setLocation(350, 200);
		clientFrame.setVisible(true);
		image = clientFrame.getToolkit().getImage(clientFrame.getClass().getClassLoader().getResource("net/img/Amy.gif"));
	...
	public void createTrayIcon(final CFrame_Amy clientFrame) {
		...
		mi.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Amy_ReadClient.isNewMessage = false;//解除托盘闪动状态
...
class SendFile implements Runnable {
	...
	public SendFile(C_Amy client, javax.swing.JFileChooser jFileChooser) {
		this.client = client;
	...
	public void run() {
		//传文件的步骤
		int count = CFrame_Amy.file_port_count;
		User2 c = CFrame_Amy.vt.get(CFrame_Amy.index);
		...
		if (info.contains("sureRec"))// 开始发送
		{
			...
			send.start();
			CFrame_Amy.jTextArea2.append("您向用户 " + c.toString() +"发送文件：" + jFileChooser.getSelectedFile().getName()+"\r\n");
		}
		socket2.close();
	}
...
class SendPoint extends Thread { // 异步_登录显示点
	public void run() {
		while (CFrame_Amy.sendPoint)// 当操作完成就停止这个
		{
			try {
				Thread.sleep(100);
			} catch (InterruptedException e) {
...
class notify2 implements Runnable {  //托盘闪动
	...
	public void run() {
		...
		while (true) {
			if (Amy_ReadClient.isNewMessage) {
				try {
					CFrame_Amy.trayIcon.setImage(new ImageIcon("").getImage());
					Thread.sleep(400);
					CFrame_Amy.trayIcon.setImage(CFrame_Amy.image);
					Thread.sleep(400);
				} catch (Exception e) {
				}
			} else
