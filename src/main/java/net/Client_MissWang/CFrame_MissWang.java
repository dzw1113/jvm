...
public class CFrame_MissWang extends javax.swing.JFrame {
	...
	private C_MissWang client = null;
	...
	public CFrame_MissWang(C_MissWang client) {
		this.client = client;
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 1) {
					if (!Morechooes) { // 单发状态
						...
						MissWang_ReadClient.isNewMessage = false;//解除托盘闪动状态//解除托盘闪动状态
		...
		if (!client.isInterruptWithServer()) { // 先判断是否服务器先关闭
			if (client.getSocket() == null) {
				...
				try {
					...
					MissWang_ReadClient.socket.close();
					isOnline = false;
			...
			MissWang_ReadClient.socket.close(); // 释放UDP的端口
	...
	public static void main(String args[]) throws InterruptedException, AWTException {
		...
		C_MissWang client = new C_MissWang();
		CFrame_MissWang clientFrame = new CFrame_MissWang(client);
		clientFrame.setTitle("Client_MissWang");
		clientFrame.setResizable(false);
		clientFrame.setLocation(350, 200);
		clientFrame.setVisible(true);
		image = clientFrame.getToolkit().getImage(clientFrame.getClass().getClassLoader().getResource("com/img/MissWang.gif"));
	...
	public void createTrayIcon(final CFrame_MissWang clientFrame) {
		...
		mi.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				MissWang_ReadClient.isNewMessage = false;//解除托盘闪动状态
...
class SendFile implements Runnable {
	...
	public SendFile(C_MissWang client, javax.swing.JFileChooser jFileChooser) {
		this.client = client;
	...
	public void run() {
		//传文件的步骤
		int count = CFrame_MissWang.file_port_count;
		User2 c = CFrame_MissWang.vt.get(CFrame_MissWang.index);
		...
		if (info.contains("sureRec"))// 开始发送
		{
			...
			send.start();
			CFrame_MissWang.jTextArea2.append("您向用户 " + c.toString() +"发送文件：" + jFileChooser.getSelectedFile().getName()+"\r\n");
		}
		socket2.close();
	}
...
class SendPoint extends Thread { // 异步_登录显示点
	public void run() {
		while (CFrame_MissWang.sendPoint)// 当操作完成就停止这个
		{
			try {
				Thread.sleep(100);
			} catch (InterruptedException e) {
			...
			CFrame_MissWang.jTextArea2.append("…");
...
class notify2 implements Runnable {  //托盘闪动
	...
	public void run() {
		...
		while (true) {
			if (MissWang_ReadClient.isNewMessage) {
				try {
					CFrame_MissWang.trayIcon.setImage(new ImageIcon("").getImage());
					Thread.sleep(400);
					CFrame_MissWang.trayIcon.setImage(CFrame_MissWang.image);
					Thread.sleep(400);
				} catch (Exception e) {
				}
			} else
